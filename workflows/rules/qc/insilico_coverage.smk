...
# Otherwise (only when running tumour only) we will try go get coverage values from the tumour sample
     ...
     rule insilico_coverage:
        ...
        run:
            ...
            insilico_level = config["insilico"][f"{wildcards.insiliconame}"]["levels"]
            shell(f"{params.pythonversion} {ROOT_DIR}/workflows/scripts/insilico_coverage/insilico_main.py --bamfile {input.bam} --bedfile {params.bedfile_path} --version {params.bedfile_version} --outputdir {wildcards.tumorid}/insilico/{wildcards.insiliconame} --annotationlevel {insilico_level} --samplename {wildcards.sname} --samtools {params.samtools}")
