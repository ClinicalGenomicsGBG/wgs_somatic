    ...
    if normalid:
        rule share_to_resultdir:
            input:
                ...
                expand("{stype}/realign/{sname}_REALIGNED.{fmt}", fmt=["bam", "bam.bai"], sname=tumorid, stype=sampleconfig[tumorname]["stype"]),
                expand("{stype}/reports/{sname}_REALIGNED.bam.tdf", sname=tumorid, stype=sampleconfig[tumorname]["stype"]),
                expand("{stype}/realign/{sname}_REALIGNED.{fmt}", fmt=["bam", "bam.bai"], sname=normalid, stype=sampleconfig[normalname]["stype"]),
